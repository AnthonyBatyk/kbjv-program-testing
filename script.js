document.addEventListener("DOMContentLoaded", () => {
  ...
  const dailyGoalEnabled = $("daily-goal-enabled");
  ...
  exportButton?.addEventListener("click",()=>{
    if(!confirm(`Експортувати базу продуктів та КБЖВ-архів?\n\nБуде експортовано ${products.length} продуктів і ${archiveItems.length} записів КБЖВ-архіву.`)){showButtonState(exportButton,"Не експортовано","error",1800);logAction("Експорт бази та архіву скасовано.");return;}
    ...
    showButtonState(exportButton,"Експортовано","success",1800);logAction(`Експортовано version-${version}: ${products.length} продуктів і ${archiveItems.length} записів КБЖВ-архіву.`);
  });
  ...
  importFile?.addEventListener("change",async()=>{
    ...
    try{
      ...
      pendingImport={parsed,normalized,hasArchive,importedArchive};
  ...
  function loadDailyGoal(){
    ...
    if(dailyGoalEnabled)dailyGoalEnabled.checked=!!dailyGoal.enabled;
  ...
  function updateDailyGoalRemaining(){
    ...
    const actual={kcal:number(kcalElement?.textContent),protein:number(proteinElement?.textContent),fat:number(fatElement?.textContent),carb:number(carbElement?.textContent),sugar:number(sugarElement?.textContent),salt:number(saltElement?.textContent),fiber:number(fiberElement?.textContent)};
    for(const key of Object.keys(remainElements)){const el=remainElements[key];if(!el)continue;const target=number(dailyGoal[key]);el.textContent=formatNumber(target-actual[key]);}
  }
  dailyGoalEnabled?.addEventListener("change",()=>{dailyGoal.enabled=dailyGoalEnabled.checked;localStorage.setItem(DAILY_GOAL_KEY,JSON.stringify(dailyGoal));renderDailyGoal();showButtonState(dailyGoalSave,dailyGoal.enabled?"Цілі увімкнено":"Цілі вимкнено",dailyGoal.enabled?"success":"error",1200);logAction(dailyGoal.enabled?"Денні цілі КБЖВ увімкнено.":"Денні цілі КБЖВ вимкнено.");});
  dailyGoalSave?.addEventListener("click",()=>{for(const key of Object.keys(goalInputs))dailyGoal[key]=Math.max(0,number(goalInputs[key]?.value));localStorage.setItem(DAILY_GOAL_KEY,JSON.stringify(dailyGoal));renderDailyGoal();showButtonState(dailyGoalSave,"Цілі збережено","success",1400);logAction("Денні цілі КБЖВ збережено.");});
  ...
  leaveSiteButton?.addEventListener("click",()=>{
    ...
    const keys=[PRODUCTS_KEY,CALCULATOR_KEY,ARCHIVE_KEY,ACTIVE_TAB_KEY,CALCULATOR_DRAFT_KEY,SORT_KEY,CONSOLE_KEY,EXPORT_VERSION_KEY,DATABASE_UPDATED_KEY,EXPORT_FINGERPRINT_KEY,DAILY_GOAL_KEY,LAST_EXPORT_KEY];
