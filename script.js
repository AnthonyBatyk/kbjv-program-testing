document.addEventListener("DOMContentLoaded", () => {
  ...
  const dailyGoalProgress = $("daily-goal-progress");
  ...
  importFile?.addEventListener("change",async()=>{
    ...
    try{
      ...
      const productKeyFn=rawProducts.every(item=>String(item?.id||"").trim())?productImportKey:(item,index)=>`name:${String(item?.name||"").trim().toLowerCase()||index}`;
      const archiveKeyFn=hasArchive&&importedArchive.every(item=>String(item?.id||"").trim())?archiveImportKey:(item,index)=>`fallback:${String(item?.date||"")}|${String(item?.text||"")}|${index}`;
      const productDiff=compareImportCollections(products,rawProducts,productKeyFn,(item,i)=>normalizeProduct(item,i));
      const archiveDiff=hasArchive?compareImportCollections(archiveItems,importedArchive,archiveKeyFn,item=>item):null;
  ...
  function renderDailyGoal(){
    ...
    const hasSavedGoal=Object.keys(goalInputs).some(key=>number(dailyGoal[key])>0);
    if(dailyGoalProgress)dailyGoalProgress.style.display=hasSavedGoal?"grid":"none";
  ...
  function renderMonthlyArchiveSummary(){
    ...
    const label=new Intl.DateTimeFormat("uk-UA",{month:"long",year:"numeric"}).format(now);
    statsMonthSummary.textContent=`${label.charAt(0).toUpperCase()+label.slice(1)}: записано ${recordedDays} днів з ${daysInMonth}.`;
  }
