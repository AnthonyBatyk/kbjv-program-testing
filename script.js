document.addEventListener("DOMContentLoaded", () => {
  ...
  let dailyGoalSettingsOpen = true;
  ...
  const checkDatabaseButton = $("check-database");
  ...
  function performPendingExport(){
    ...
    const data={version,exported_at:exportedAt,products:products.map((p,i)=>normalizeProduct(p,i)),archive:archiveItems};
    ...
    localStorage.setItem(LAST_EXPORT_KEY,new Date().toISOString());updateSiteDataCounts();
    closeExportPreview();
    showButtonState(exportButton,"Експортовано","success",1800);
    logAction(`Експортовано version-${version}: ${products.length} продуктів і ${archiveItems.length} записів КБЖВ-архіву.`);
  }
  exportButton?.addEventListener("click",()=>{
    const version=peekExportVersion(),exportedAt=new Date().toISOString();
    pendingExport={version,exportedAt};
    if(exportPreviewProducts)exportPreviewProducts.textContent=String(products.length);
    if(exportPreviewArchive)exportPreviewArchive.textContent=String(archiveItems.length);
    ...
    exportPreviewModal?.classList.add("active");document.body.classList.add("edit-modal-open");
    logAction(`Перевірка експорту відкрита: ${products.length} продуктів, ${archiveItems.length} записів архіву, version-${version}. Очікується підтвердження.`);
  });
  exportPreviewCancel?.addEventListener("click",()=>{
    ...
    logAction("Експорт бази та архіву скасовано після перевірки.");
  ...
  function closeImportPreview(){importPreviewModal?.classList.remove("active");document.body.classList.remove("edit-modal-open");pendingImport=null;}
  function applyPendingImport(){
    if(!pendingImport)return;const {parsed,normalized,hasArchive,importedArchive}=pendingImport;
    saveUndoSnapshot("Імпорт бази та архіву");
    products=normalized;if(hasArchive)archiveItems=importedArchive;
    ...
    updateSiteDataCounts();
    closeImportPreview();showButtonState(importButton,"Імпортовано","success",1500);logAction(`Імпортовано ${products.length} продуктів${hasArchive?` і ${archiveItems.length} записів КБЖВ-архіву`:"; КБЖВ-архів не змінювався"}.`);
    if(importFile)importFile.value="";importDialogOpened=false;
  }
  importPreviewCancel?.addEventListener("click",()=>{closeImportPreview();if(importFile)importFile.value="";importDialogOpened=false;showButtonState(importButton,"Не імпортовано","error",1500);showButtonState(importPreviewCancel,"Скасовано","error",1000);logAction("Імпорт бази скасовано після перевірки файла.");});
  importPreviewConfirm?.addEventListener("click",()=>{showButtonState(importPreviewConfirm,"Імпортовано","success",700);setTimeout(applyPendingImport,250);});
  ...
  importFile?.addEventListener("change",async()=>{
    ...
    try{
      ...
      const productDiff=compareImportCollections(products,rawProducts,productImportKey,(item,i)=>normalizeProduct(item,i));
      const archiveDiff=hasArchive?compareImportCollections(archiveItems,importedArchive,archiveImportKey,item=>item):null;
      pendingImport={parsed,normalized,hasArchive,importedArchive,productDiff,archiveDiff};
      if(importPreviewProducts)importPreviewProducts.textContent=String(normalized.length);
      if(importPreviewProductsDiff)importPreviewProductsDiff.textContent=formatImportDiff(productDiff);
      if(importPreviewArchive)importPreviewArchive.textContent=hasArchive?String(importedArchive.length):"Не містить архів";
      if(importPreviewArchiveDiff)importPreviewArchiveDiff.textContent=hasArchive?formatImportDiff(archiveDiff):"Архів не змінюється";
      if(importPreviewVersion)importPreviewVersion.textContent=!Array.isArray(parsed)&&parsed.version!=null?String(parsed.version):"Не вказано";
      if(importPreviewDate)importPreviewDate.textContent=!Array.isArray(parsed)?formatImportDate(parsed.exported_at):"Не вказано";
      importPreviewModal?.classList.add("active");document.body.classList.add("edit-modal-open");
      logAction(`Файл імпорту перевірено: ${normalized.length} продуктів${hasArchive?`, ${importedArchive.length} записів архіву`:"; без архіву"}. Очікується підтвердження.`);
    }catch(e){console.error(e);pendingImport=null;showButtonState(importButton,"Не імпортовано","error",1500);logAction("Помилка перевірки файла імпорту.");alert("Не вдалося імпортувати базу.\n\nПеревірте JSON-файл.");if(importFile)importFile.value="";importDialogOpened=false;}
  });
  ...
  function checkDatabaseIntegrity(){
    ...
    function readArray(key,label){
      const raw=localStorage.getItem(key);
      if(raw===null)return [];
      try{
        const value=JSON.parse(raw);
        if(!Array.isArray(value)){issues.push(`${label}: дані мають неправильний формат.`);return null;}
        return value;
      }catch(_){
        issues.push(`${label}: пошкоджений JSON у локальному сховищі.`);
        return null;
      }
    }

    const storedProducts=readArray(PRODUCTS_KEY,"База продуктів");
    const storedArchive=readArray(ARCHIVE_KEY,"КБЖВ-архів");

    if(storedProducts){
      storedProducts.forEach((p,index)=>{
        const pos=index+1;
        if(!p||typeof p!=="object"){issues.push(`Продукт №${pos}: некоректний запис.`);return;}
        const name=String(p.name||"").trim();
        if(!name)issues.push(`Продукт №${pos}: відсутня назва.`);
        const id=String(p.id||"").trim();
        if(id){
          if(productIds.has(id))issues.push(`Дублікат ID продукту: ${id}.`);
          productIds.add(id);
        }
        if(name){
          const key=name.toLocaleLowerCase("uk-UA");
          if(productNames.has(key))duplicateNames.push(`«${name}»`);
          else productNames.set(key,pos);
        }
        ["kcal","protein","fat","carb","sugar","salt","fiber"].forEach(field=>{
          const rawValue=field==="carb"?(p.carb??p.carbs):p[field];
          if(rawValue===undefined||rawValue===null||rawValue==="")return;
          const value=Number(rawValue);
          if(!Number.isFinite(value)||value<0)issues.push(`Продукт «${name||pos}»: некоректне число у полі ${field}.`);
        });
      });
      if(duplicateNames.length)issues.push(`Дублікати назв продуктів: ${[...new Set(duplicateNames)].join(", ")}.`);
    }

    if(storedArchive){
      storedArchive.forEach((item,index)=>{
        const pos=index+1;
        if(!item||typeof item!=="object"){issues.push(`Архів №${pos}: некоректний запис.`);return;}
        const id=String(item.id||"").trim();
        if(id){
          if(archiveIds.has(id))issues.push(`Дублікат ID архіву: ${id}.`);
          archiveIds.add(id);
        }
        const date=String(item.date||"");
        if(date&&!/^\d{4}-\d{2}-\d{2}$/.test(date))issues.push(`Архів №${pos}: некоректна дата «${date}».`);
        ["kcal","protein","fat","carb","sugar","salt","fiber"].forEach(field=>{
          if(item[field]===undefined||item[field]===null||item[field]==="")return;
          const value=Number(item[field]);
          if(!Number.isFinite(value)||value<0)issues.push(`Архів №${pos}: некоректне число у полі ${field}.`);
        });
      });
    }

    if(issues.length===0){
      showButtonState(checkDatabaseButton,"База справна","success",1800);
      logAction(`Перевірка бази завершена: помилок не знайдено. Продуктів: ${storedProducts?.length||0}, записів архіву: ${storedArchive?.length||0}, дублікатів: 0.`);
      return;
    }

    showButtonState(checkDatabaseButton,`Проблем: ${issues.length}`,"error",2500);
    logAction(`Перевірка бази: знайдено проблем — ${issues.length}. ${issues.join(" | ")}`);
  }
  checkDatabaseButton?.addEventListener("click",checkDatabaseIntegrity);
